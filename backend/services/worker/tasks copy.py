...
<process>
1. WATCH every frame. For each, note its shot type (hook / product-display / close-up / on-body-demo / full-body-OOTD / back-view / reaction / cta-closing) and mark USE or REJECT.
2. UNDERSTAND the clip before writing: what is the product, what is the creator doing, what is the story. Do not write a single line until you understand the footage.
3. SCRIPT: write a coherent Thai voiceover — hook → product intro → features/demo → full look → CTA. Each line describes ONLY what its matched frame actually shows. If no frame supports a claim, do not write that line.
4. MATCH each line to the best frame(s). Total 45–60s (45s is a hard floor).
5. VERIFY in English: durationSec sum ≥ 45s; the last line is a CTA; no two adjacent lines look the same.
</process>
...
<reject_prep>
Skip any frame where the creator is: fixing hair, adjusting or smoothing the outfit, reaching for or touching the camera, setting up, looking away, mid-step into a pose, or not yet ready. Use only settled, intentional, camera-ready moments.
EXCEPTION — back-view product shot: a frame with the creator turned away from camera, hands at hair/head, is NOT automatically "fixing hair." Look at what's in frame: if the garment's back design (neckline, straps, back pattern/logo) is clearly visible and the pose is settled (not mid-turn, not blurry), classify it as a "back-view" product shot and USE it — back design is a real selling point, do not discard it just because the hands are near the head.
</reject_prep>

<shots>
- Classify each kept frame's type before matching.
- Important shots (product reveal, full-look OOTD, on-body demo, hero close-up) must be shown COMPLETE — give them their full duration, never cut mid-action.
- Match script to real frames only: the line must match what the frame shows.
</shots>

<variety>
- Every line must look VISUALLY DIFFERENT from the line before it — change distance (close-up vs full body), angle (front/side/back), or subject focus. This applies to single-shot lines too, not just multi-angle.
- Two cuts of the same pose + same distance a few seconds apart look identical = forbidden.
- Multi-angle cuts: pick frames far apart in time (≥30s) so the angle genuinely changes. Same pose + same distance = NOT multi-angle.
- Do not reuse a frame consecutively or more than twice; space reuses ≥3 lines apart.
</variety>

<timing>
- Single-shot (hook / CTA / commentary): one cut, ≤3.0s.
- Multi-angle (product / OOTD / demo): 2–3 cuts, each 0.8–1.0s.
- Match durationSec to the moment's weight: key reveal / full-body OOTD 2.5–3.0s; normal commentary 2.0–2.5s; quick flash / reaction 1.5–2.0s. Never use one duration for every line.
- sourceIn within ±0.35s of matchedFrameTime.
</timing>

<script_quality>
- Hook: the first line (0–3s) must grab attention — not a generic stand-still intro.
- Coherent flow, not random lines. Do not repeat a feature you already mentioned — move to the next point.
...
- Source: full user_script → keep wording, split into 2–5s lines. Brief only → write from brief + frames. Neither → infer from frames.
</script_quality>
...
Return ONLY a valid JSON object, no prose or markdown. totalEstimatedSec = sum of all durationSec.
{
  ...
  "segments": [
    ...
    },
    {
      ...
      "sourceClip": "clip0", "sourceIn": 40.0, "sourceOut": 40.9, "durationSec": 0.9,
      "matchedFrameTime": 40.0, "visualDescription": "close-up เนื้อสินค้า",
...
async def analyze_dub_first(ctx: dict[str, Any], *, job_id: str, project_uid: str, tenant_slug: str) -> dict:
    ...
    try:
        ...
        from packages.video.scene import (
            build_vision_content_uploaded,
            extract_dub_budget_frames,
            extract_edge_frames,
        ...
        vision_content, vision_stats, uploaded_file_ids = await build_vision_content_uploaded(
            all_frames,
        )
        ...
        duration_hint = (
            f"Target video length: ~{target_duration_sec} seconds. totalEstimatedSec = sum of ALL segment durationSec = actual rendered video length. Plan enough lines so all cuts total ~{target_duration_sec}s — add more lines if needed. "
            if target_duration_sec
            else "No target set — minimum 45s, target 45–60s (standard TikTok affiliate length). totalEstimatedSec = sum of ALL segment durationSec = actual rendered video length. 45s is a hard floor — plan at least 15 lines and keep adding until the sum reaches 45s. "
        )
        creator_input = (
            f"<creator_input>\n"
        ...
        user_msg_content: list[dict[str, Any]] = [{"type": "text", "text": (
            ...
            "Follow all system rules (safety, no-prep, frame-match, shot completeness, visual variety, timing, CTA). "
        ...
        log.info(
            ...
            payload_approx_kb=vision_stats.get("message_payload_kb", 0) + round(text_chars / 1024),
            transport=vision_stats.get("transport"),
            upload_ms=vision_stats.get("upload_ms"),
            note="upload_ms=Files API; message body uses file_id refs; API wait logged as llm_acompletion_waiting",
        )
        ...
        try:
            resp = await acompletion_stream_thinking(
                messages, system=_DUB_EDIT_SYSTEM, project_uid=project_uid,
                on_thinking=_push_thinking, **vx
            )
...
async def render_dub_silent(ctx: dict[str, Any], *, job_id: str, project_uid: str, tenant_slug: str) -> dict:
    ...
    try:
        ...
        clips_dir = output_dir / "clips"
        clips_dir.mkdir(exist_ok=True)
        # Drop per-scene clips from prior renders so clip count always matches segments.
        for stale in clips_dir.glob("clip_*.mp4"):
            stale.unlink(missing_ok=True)
