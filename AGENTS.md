...
- **`backend/packages/`** — shared libs:
  ...
  - `db/` — `base.py`, `session.py`, `config.py`, `upserts.py`, `tenancy.py` (schema management).
    ...
    - `models/video_project.py` — VideoProject (per-tenant; statuses incl. pending/processing/waiting_vo/done/error/paused_quota/cancelled; modes: talking_head/dub_first/highlight/speech_highlights/speech_scenes).
