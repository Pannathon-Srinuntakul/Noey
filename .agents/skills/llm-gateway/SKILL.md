---
name: llm-gateway
description: Provider-agnostic AI usage via packages/llm (LiteLLM) — cloud (Codex/OpenAI/Gemini) and local (Ollama/vLLM). Load before writing any AI/LLM/chatbot/prompt-cron code, or adding a provider.
---
...
single gateway over **LiteLLM** gives one interface across Codex, OpenAI, Gemini, and
...
- `LLM_MODEL` — e.g. `anthropic/Codex-opus-4-8` (default), `openai/gpt-...`,
...
- **Default model `Codex-opus-4-8`** unless the user sets `LLM_MODEL` otherwise.
